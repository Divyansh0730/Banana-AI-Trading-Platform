...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await fetch('http://20.244.83.76:8000/api/v1/auth/register', {
        method: 'POST',
    ...
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden animate-fade-in">
      ...
      <div className="w-full max-w-md p-8 glass-card border border-white/10 relative z-10 flex flex-col gap-6">
        <div className="text-center mb-4">
          ...
          <h1 className="text-3xl font-bold text-white tracking-wider">Join BANANA AI</h1>
          <p className="text-gray-400 text-sm mt-2">Create your algorithmic trading account</p>
        </div>
        ...
        <div className="text-center text-sm text-gray-500 mt-2">
          Already have an account? <Link href="/login" className="text-banana hover:underline">Log in</Link>
        </div>
      </div>
    </div>
  );
